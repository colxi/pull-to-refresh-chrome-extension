/*
 *
 * Pull to refresh Chrome extension
 * https://chrome.google.com/webstore/detail/pull-to-refresh/opiolhkaggfkacemkinlcbidbidfpenl
 * @Author: colxi
 *
 */

document.addEventListener('DOMContentLoaded', function(event) {
  // According to the standard if overscrollBehaviorY CSS property is set
  // to contain, no page reload should be performed on overscroll
  if (getComputedStyle(document.body).overscrollBehaviorY === 'contain') {
    console.info(
      'Pull to refresh disabled in this website. ( body.styles.overscroll-behavior-y = contain )'
    )
    return false
  }

  // inject in DOM '<div id="reload_chrome_ext"><img src="img/re.png"></div>'
  var div = document.createElement('div')
  div.id = 'reload_chrome_ext'
  var img = document.createElement('img')
  img.src =
    'data:image/png;base64,'
  div.appendChild(img)
  document.body.appendChild(div)

  let host = document.location.hostname
  var DEBUG = host === '127.0.0.1' || host === 'localhost' ? true : false
  var timeout = 2
  let touchstartX = 0
  let touchstartY = 0
  let themeBaseColor = '#f50057'
  // gestures navigation
  initiateReload = settings => {
    let mouseSens = 1.0
    let touchSens = 1.0
    console.log(settings)
    if (typeof settings === 'object' && settings.sensitivitySettings) {
      mouseSens = settings.sensitivitySettings.mouseSens
      touchSens = settings.sensitivitySettings.touchSens
    }
    var gestures = {
      DOMElement: document.querySelector('#reload_chrome_ext'),
      sensitivity: {
        wheel: Number(mouseSens),
        touchmove: Number(touchSens)
      },
      actionAmountLimit: 120,
      actionAmount: 0,
      timeoutCountdown: timeout,
      timeoutWatcher: null,
      log: function(msg) {
        if (DEBUG) console.log(msg)
        return true
      },
      getDirection: function(e) {
        // Detect direction in both e.type cases ( wheel  touchmove )
        let direction = ''
        if (e.type === 'wheel') direction = e.deltaY < 0 ? 'up' : 'down'
        else {
          let dif_x = Math.abs(touchstartY - e.touches[0].screenY)
          let dif_y = Math.abs(touchstartX - e.touches[0].screenX)
          let axis = dif_x >= dif_y ? 'vertical' : 'horizontal'
          if (axis === 'horizontal') {
            direction =
              touchstartX - e.touches[0].screenX < 0 ? 'left' : 'right'
          } else
            direction = touchstartY - e.touches[0].screenY < 0 ? 'up' : 'down'
        }
        //gestures.log(direction);
        return direction
      },
      getDOMElementPosition: function() {
        return parseInt(gestures.DOMElement.style.top, 10)
      },
      timeoutCheck: function() {
        if (gestures.timeoutWatcher === null) {
          gestures.log('timeout countdown created')
          gestures.timeoutWatcher = setInterval(function() {
            if (gestures.getDOMElementPosition() < -50) {
              gestures.log('timeout countdown destroyed')
              clearInterval(gestures.timeoutWatcher)
              gestures.timeoutWatcher = null
              return
            }
            if (gestures.timeoutCountdown > 0) gestures.timeoutCountdown -= 0.1
            else {
              gestures.timeoutCountdown = 0
              gestures.actionAmount--
              gestures.render()
            }
          }, 10)
        }
      },
      render: function() {
        gestures.DOMElement.style.top = -50 + gestures.actionAmount + 'px'
        gestures.DOMElement.style.transform =
          'rotate(' + gestures.actionAmount * 2 + 'deg)'
        gestures.DOMElement.style.opacity = (gestures.actionAmount * 2) / 100
      },
      isOverscroll(e) {
        if (window.pageYOffset !== 0) {
          gestures.log('Window scroll not 0. Cancelled')
          return false
        }
        if (e.scrollTop !== 0) {
          gestures.log('Element scroll not 0. Cancelled')
          return false
        }
        gestures.log('Window overscroll!')
        while (e && e !== document.body) {
          e = e.parentNode
          if (e === document.body) break
          if (e.scrollTop !== 0) return false
        }
        return true
      },
      isReloading() {
        return gestures.DOMElement.hasAttribute('reloading_chrome_ext')
      },
      scrollEventHandler: function(e) {
        //console.log(e)
        // if direction is down, cancel reload event progress
        if (gestures.getDirection(e) === 'down' || gestures.isReloading()) {
          gestures.timeoutCountdown = 0
          return false
        }
        if (gestures.isOverscroll(e.target)) {
          gestures.timeoutCheck()
          // increase actionAmount
          gestures.actionAmount += gestures.sensitivity[e.type]

          if (gestures.actionAmount > gestures.actionAmountLimit) {
            gestures.actionAmount = 90
            gestures.render()
            gestures.DOMElement.setAttribute('reloading_chrome_ext', 'true')
            setTimeout(function() {
              location.reload()
            }, 1000)
            clearInterval(gestures.timeoutWatcher)
            return 'RELOADIND_PAGE'
            // END OF EXECUTION, PAGE RELOADS
          }
          // set the positon of the loader element
          gestures.render()
          // reset the timeoy counter
          gestures.timeoutCountdown = timeout
        }
      }
    }

    /* Add the event listeners for each event. */
    document.addEventListener('wheel', gestures.scrollEventHandler, {
      passive: true
    })
    document.addEventListener('touchmove', gestures.scrollEventHandler, {
      passive: true
    })
    document.addEventListener(
      'touchstart',
      function(event) {
        touchstartX = event.changedTouches[0].screenX
        touchstartY = event.changedTouches[0].screenY
      },
      false,
      { passive: true }
    )
  }

  chrome.storage.local.get('sensitivitySettings', initiateReload)
})
